Shader "Custom/HexConcrete"
{
    Properties
    {
        // Surface colours — two variants so core and fringe can share
        // this shader but look different by adjusting the material.
        _ConcreteColor   ("Concrete Color",         Color)        = (0.52, 0.52, 0.54, 1)
        _SideColor       ("Side Color",             Color)        = (0.40, 0.38, 0.36, 1)

        // Border
        _BorderColor     ("Border Color",           Color)        = (0.92, 0.92, 0.92, 1)
        _BorderWidth     ("Border Width",           Range(0.01, 0.3)) = 0.07

        // Hex geometry — must match the value used in HexRoad
        _HexInradius     ("Hex Inradius",           Float)        = 0.85

        // Top-face detection
        _TopThreshold    ("Top Face Threshold",     Range(0.1, 1.0)) = 0.85

        // Set per-tile from GridManager.UpdateConcreteBorders() via MaterialPropertyBlock.
        // IMPORTANT: both properties are declared OUTSIDE UnityPerMaterial below so
        // the SRP Batcher does not own them — required for MaterialPropertyBlock to work.
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                // GridManager.UpdateConcreteBorders() via MaterialPropertyBlock.
                ...
                // Borders only on the top face
                isBorder *= isTop;

                // ── Colour composition ────────────────────────────────────
                half3 col = lerp(_SideColor.rgb, _ConcreteColor.rgb, isTop);
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                half4  _ConcreteColor; half4 _SideColor; half4 _BorderColor;
                half   _BorderWidth;   half  _HexInradius; half _TopThreshold;
            CBUFFER_END
